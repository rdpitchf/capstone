...
class OV7670_SCCB{
  public:
    ...
    static bool write_until_true_saturation(uint8_t subaddress, uint8_t data){
      uint8_t read_data = read(subaddress);
      // printf("1 read data %d write data %d subaddress %d \r\n", read_data, data, subaddress);
      while((read_data >> 4) != (data >> 4)){
        // wait_us(1000000);
        bool status = write_3_phase(subaddress, data);
        // wait_us(1000000);
        read_data = read(subaddress);
        // printf("2 read data %d write data %d subaddress %d status %d \r\n", read_data, data, subaddress, status);
      }
      return true;
    }
    ...
    static bool write_3_phase(uint8_t subaddress, uint8_t data){
      ...
      if(!write_addresses(ID_ADDRESS_WRITE)){
        success = false;
        printf("FAILED ID_ADDRESS_WRITE\r\n");
      }

      if(!write_addresses(subaddress)){
        success = false;
        printf("FAILED subaddress\r\n");
      }
