    ...
    void ReadTransmitCapturedFrame(){
      ...
      for (int height = 0; height < PHOTO_HEIGHT; height++){
        for (int width = 0; width < PHOTO_WIDTH; width++){
          ...
          frame_pic[frame_counter++] = PixelData;

          wait_us(1); // Pauses for DurationMicroSecs microseconds
          rclk.write(0); // Sets the pin off
          wait_us(1);
      ...
      serial_port->write(frame_pic, frame_counter);
      printf("\r\n<<<<< END >>>>> \r\n");
