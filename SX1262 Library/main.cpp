...
uint8_t *request_tx_packet[REQUEST_PACKET_SIZE];
uint8_t *request_grant_rx_packet[REQUEST_GRANT_PACKET_SIZE];
int16_t request_grant_rx_rssi;
int8_t request_grant_rx_snr;
...
uint8_t *central_hub_request_rx_packet[REQUEST_PACKET_SIZE];
uint8_t *central_hub_request_grant_tx_packet[REQUEST_GRANT_PACKET_SIZE];
int16_t central_hub_request_rx_rssi;
int8_t central_hub_request_rx_snr;
volatile central_hub_request_states_t central_hub_request_state = INIT;
// ************** For Central Hub **************
  ...
  while(1){
    switch(request_state){
      case INIT:{
        ...
        request_tx_packet[0] = RADIO_ID >> 8;
        request_tx_packet[1] = RADIO_ID & 0x00FF;
...
uint8_t central_hub_request_data(uint8_t timeout_seconds){
  ...
  uint16_t trail_camera_id;
  uint8_t trail_camera_battery_enable;
  uint8_t trail_camera_battery_status;
...
void On_rx_done(const uint8_t *payload, uint16_t size, int16_t rssi, int8_t snr){
  ...
  switch(request_state){
    case WAIT_RECEIVE_DONE:{
      if(size <= REQUEST_GRANT_PACKET_SIZE){
    ...
    switch (inputBuffer[0]){
      ...
      case 'b':
        ...
        if(is_trail_camera){
          ...
          print("send battery status result: %d", result);
        }
        led1 = 0;
        break;
