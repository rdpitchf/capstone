...
static uint8_t sd_data[DATA_PACKET_SIZE-3]{0};
...
static request_states_t tc_request_state = INIT;
static request_states_t ch_request_state = INIT;
static request_states_t tc_data_state = INIT;
static request_states_t ch_data_state = INIT;

static RadioFlags_t radioFlags{
    .rxDone = false,
    .rxError = false,
    .txDone = false,
    .rxTimeout = false,
...
class Sx_radio{
  ...
  public:
    ...
    PinName dio1 = PG_1;
    PinName busy = PF_9;
    PinName freq_select = PC_5;
    PinName device_select = PC_4;
    PinName crystal_select = PE_3;
    PinName ant_switch = PE_6;
    ...
    ~Sx_radio(){
      delete this->radio;
      delete this->radio_events;
      this->radio = nullptr;
      this->radio_events = nullptr;
    }
    Sx_radio(Micro_sd_card *sd_card_ref, BufferedSerial *serial_port_ref){
      this->radio = new SX126X_LoRaRadio(this->mosi, this->miso, this->sclk, this->nss, this->reset, this->dio1, this->busy, this->freq_select, this->device_select, this->crystal_select, this->ant_switch);
      this->radio_events = new radio_events_t{
        .tx_done = &Sx_radio::On_tx_done,
        .tx_timeout = &Sx_radio::On_tx_timeout,
        .rx_done = &Sx_radio::On_rx_done,
        .rx_timeout = &Sx_radio::On_rx_timeout,
        .rx_error = &Sx_radio::On_rx_error,
        .fhss_change_channel = &Sx_radio::On_fhss_change_channel,
        .cad_done = &Sx_radio::On_cad_done,
      };
      this->radio->radio_reset();
      this->radio->init_radio(radio_events);
      this->sd_card = sd_card_ref;
      this->serial_port = serial_port_ref;
    }
    ...
    uint8_t tc_tx_data_buffer_size = 0;
    ...
    void tc_get_data_from_device(uint8_t max_size){
      ...
      if(!this->sd_card->end_of_file()){
        size_t number_bytes = this->sd_card->read_data_from_file(sd_data, max_size);
        tc_tx_data_buffer_size = number_bytes;
        for(uint8_t i = 0; i < number_bytes; i++){
          tc_data_tx_packet[i+3] = sd_data[i];
        }
      }else{
    ...
    void ch_print_data_to_cpu(uint8_t size_of_data){
      printf("<<<< OUTPUT DATA START %d Bytes >>>>\r\n", size_of_data);
      uint8_t frame[1]{0};
      for(uint8_t i = 0; i < size_of_data; i++){
        frame[0] = ch_data_rx_packet[i+3];
        serial_port->write(frame, 1);
      }
      if(size_of_data == 0){
        printf("<<<< OUTPUT DATA STOP >>>>\r\n");
      }
    ...
    uint8_t ch_recevie_data(uint16_t device_id, uint16_t data_size){
      ...
      printf("<<<< START RECORD PICTURE DATA >>>>\r\n");
      uint8_t interval_counter = 0;
    ...
    static void On_rx_done(const uint8_t *payload, uint16_t size, int16_t rssi, int8_t snr){
      printf("rx_done. \r\n");
    ...
    static void On_fhss_change_channel(uint8_t current_channel){
      printf("fhss_change_channel. \r\n");
    }
    static void On_cad_done(bool channel_busy){
      printf("cad_done. \r\n");
    }
    ...
    void set_radio_configuration_default(bool lora_or_fsk){
      ...
      if(using_lora == 1){
        ...
        set_tx_settings_lora(23, 0, 2, 10, 1, 12, false, true, true, 1, false, 1000);
