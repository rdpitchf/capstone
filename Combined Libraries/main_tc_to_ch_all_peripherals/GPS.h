...
#define defaultMaxWait 1100
	...
	geofenceParams currentGeofenceParams;

	void printPacket(ubxPacket *packet); //Useful for debugging
	void enableDebugging(bool printLimitedDebug);
...
#define getDOPmaxWait 1100 // Default maxWait for getDOP and all functions which all it
	...
	position_time_date_t position_time_date(uint16_t maxWait = getPVTmaxWait);
	...
	int32_t getLongitude(uint16_t maxWait = getPVTmaxWait);
	int32_t getAltitude(uint16_t maxWait = getPVTmaxWait);
	uint32_t getTimeOfWeek(uint16_t maxWait = getPVTmaxWait /* = 250*/);
	...
	bool begin(I2C  &wirePort, uint8_t deviceAddress);
